...
Complete step-by-step guide to deploy Exnotic video platform on Vercel using the Express backend. No YouTube API keys required.
...
- **`buildCommand: "npm run build"`**: Builds both frontend (Vite) and backend (Express server)
- **`functions`**: Configures the Express server as a Vercel function
- **`routes`**: Routes all requests to the Express server, which handles both API and static files

Note: The Express server serves both the frontend static files and API endpoints, maintaining all functionality from Replit.
